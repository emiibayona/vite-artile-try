  ...
  <div>
    <UpperBarGeneric
      ...
      :config="{
        ...
        secondInput: { show: true, label: 'Proveedor' },
        date: { show: true, label: 'Fecha from' },
        secondDate: { show: true, label: 'Fecha hasta', avoidInit: true },
        dropdown: {
          show: true,
          label: 'Institucion',
          options: centersList,
...
async function searchMovements({
  ...
  actualParams.value = {
    search,
    secondSearch,
    from: date ? moment(date).toISOString() : null,
    to: secondDate ? moment(secondDate).toISOString() : null,
    type: PAYMENT_TYPES.value.OUTCOME,
    centerId: dropdown,
  };
